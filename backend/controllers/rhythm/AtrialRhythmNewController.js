...
async function Confirm(req, res) {
  try {
    ...
    if (LogedUser && Id) {
      const Atrial = await Models.AtrialRhythmNew.findOne({
        where: { Id },
        raw: true,
      });
      if (!Atrial) {
      ...
      await Models.AtrialRhythmNew.update({ is_confirm: 'yes' }, { where: { Id } });
  ...
  try {
    ...
    if (LogedUser && PatientRegNo) {
      const [LastData, data] = await sequelize.query(
        "SELECT TOP 1 Id FROM AtrialRhythmNew WHERE PatRegNo=:PatientRegNo AND (is_confirm='no' OR is_confirm IS NULL) ORDER BY Id DESC",
...
async function CustomSave(req, res) {
  try {
    ...
    if (PatientRegNo && LogedUser && Data) {
      //create hf ambulance
      let AtrialRhythmNewId = null;
      if (Id) {
        AtrialRhythmNewId = await BaseControllerHelper.BaseUpdate({
          ObjectName: 'AtrialRhythmNew',
          Data: { ...Data, Id },
          LogedUser,
          SaveLog: true,
        ...
        // create AtrialRhythmNew
        AtrialRhythmNewId = await BaseControllerHelper.BaseCreate({
          ObjectName: 'AtrialRhythmNew',
          Data: {
            ...Data,
            is_confirm: 'no',
      ...
      result.Data = { DataId: AtrialRhythmNewId };
      return res.send(JSON.stringify(result));
    } else {
      return res.send(
        JSON.stringify(BaseControllerHelper.GetDefaultErrorResult('Information is missing'))
