...
router.post('/checkConfirm', checkConfirm);
...
async function Confirm(req, res) {
  try {
    ...
    if (LogedUser && Id) {
      const Atrial = await Models.AtrialRhythm.findOne({
        where: { Id },
        raw: true,
      });
      if (!Atrial) {
      ...
      await Models.AtrialRhythm.update({ is_confirm: 'yes' }, { where: { Id } });
  ...
  try {
    ...
    if (LogedUser && PatientRegNo) {
      const [LastData, data] = await sequelize.query(
        'SELECT TOP 1 Id FROM AtrialRhythm WHERE PatRegNo=:PatientRegNo AND (is_confirm=\'no\' OR is_confirm IS NULL) ORDER BY Id DESC',
...
async function CustomSave(req, res) {
  try {
    ...
    if (PatientRegNo && LogedUser && Data) {
      //create hf ambulance
      let AtrialRhythmId = null;
      if (Id) {
        AtrialRhythmId = await BaseControllerHelper.BaseUpdate({
          ObjectName: 'AtrialRhythm',
          Data: { ...Data, Id },
          LogedUser,
          SaveLog: true,
        ...
        // create AtrialRhythm
        AtrialRhythmId = await BaseControllerHelper.BaseCreate({
          ObjectName: 'AtrialRhythm',
          Data: {
            ...Data,
            is_confirm: 'no',
      ...
      result.Data = { DataId: AtrialRhythmId };
      return res.send(JSON.stringify(result));
    } else {
      return res.send(
        JSON.stringify(BaseControllerHelper.GetDefaultErrorResult('Information is missing'))
...
async function checkConfirm(req, res) {
  ...
  try {
    ...
    if (LogedUser && Id) {
      ...
      const Record = await Models.AtrialRhythm.findOne({
        where: { Id, PatRegNo },
