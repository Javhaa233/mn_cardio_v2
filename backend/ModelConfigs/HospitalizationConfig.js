const { Models } = require('../config/DB');
const Model = Models.Hospitalization;

function HospitalizationConfig() {
  this.Fields = [
    [
      {
        Name: 'conclusion',
        Label: 'Conclusion',
        Type: 'Text',
        md: 4,
        Position: 1,
      },
      {
        Name: 'date_creation',
        Label: 'Date de cr',
        Type: 'Date',
        md: 4,
        Position: 2,
      },
      {
        Name: 'date_modif',
        Label: 'Date de modification',
        Type: 'Date',
        md: 4,
        Position: 3,
      },
      {
        Name: 'future_treatment',
        Label: 'Future treatment',
        Type: 'Text',
        md: 4,
        Position: 4,
      },
      {
        Name: 'id',
        Label: "Identifiant d'utilisateur",
        Type: 'Text',
        md: 4,
        Position: 5,
      },
      {
        Name: 'id_data',
        Label: 'Identifiant de la fiche',
        Type: 'Text',
        md: 4,
        Position: 6,
      },
      { Name: 'id_group', Label: 'Groupe', Type: 'Text', md: 4, Position: 7 },
      {
        Name: 'rec_status',
        Label: 'Status',
        Type: 'RadioBox',
        Config: { IdField: 'Value', TextField: 'Label' },
        md: 4,
        OptionType: 'rec_status',
        Position: 10,
      },
      {
        Name: 'severity_admission',
        Label: 'Severity on admission',
        Type: 'RadioBox',
        Config: { IdField: 'Value', TextField: 'Label' },
        md: 4,
        OptionType: 'severity',
        Position: 12,
      },
      {
        Name: 'user_mod',
        Label: 'Auteur de la derni',
        Type: 'Text',
        md: 4,
        Position: 13,
      },
    ],
  ];

  this.ObjectName = 'Hospitalization';
  this.Model = Model;
  this.OptionTypes = Models.OptionTypes;
  this.PK = 'id_data';
  this.NewObject = {};
  this.TitleObject = {
    Title: 'Hospitalization',
    NewObjectTitle: 'Hospitalization create',
    EditObjectTitle: 'Hospitalization edit',
  };
}

module.exports = HospitalizationConfig;
