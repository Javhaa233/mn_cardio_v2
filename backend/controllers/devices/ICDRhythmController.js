...
async function Confirm(req, res) {
  try {
    ...
    if (LogedUser && Id) {
      const Atrial = await Models.ICDRhythm.findOne({
        where: { Id },
        raw: true,
      });
      if (!Atrial) {
      ...
      await Models.ICDRhythm.update({ is_confirm: 'yes' }, { where: { Id } });
  ...
  try {
    ...
    if (LogedUser && PatientRegNo) {
      const [LastData, data] = await sequelize.query(
        "SELECT TOP 1 Id FROM ICDRhythm WHERE PatRegNo=:PatientRegNo AND (is_confirm='no' OR is_confirm IS NULL) ORDER BY Id DESC",
      ...
      if (LastData.length === 1) {
        if (LastData[0].Id) {
          const Option = {
            ...
            limit: 1,
            offset: 0,
            SearchField: [{ Field: 'Id', Value: LastData[0].Id, Op: 'Equals' }],
            FindType: 'AllData',
            WhereType: '',
          };
...
async function CustomSave(req, res) {
  try {
    ...
    if (PatientRegNo && LogedUser && Data) {
      //create hf ambulance
      let ICDRhythmId = null;
      if (Id) {
        ICDRhythmId = await BaseControllerHelper.BaseUpdate({
          ObjectName: 'ICDRhythm',
          Data: { ...Data, Id },
          LogedUser,
          SaveLog: true,
        ...
        // create ICDRhythm
        ICDRhythmId = await BaseControllerHelper.BaseCreate({
          ObjectName: 'ICDRhythm',
      ...
      result.Data = { DataId: ICDRhythmId };
      return res.send(JSON.stringify(result));
    } else {
      return res.send(
        JSON.stringify(BaseControllerHelper.GetDefaultErrorResult('Information is missing'))
