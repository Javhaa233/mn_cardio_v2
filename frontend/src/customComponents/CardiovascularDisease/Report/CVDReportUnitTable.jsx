...
import { HeaderArray, RowColumns } from "assets/store/reportUnitTableColumns";
...
class CVDReportUnitTable extends Component {
  constructor(props) {
    super(props);
  ...
  // Нэгдсэн бүртгэлийн тайлан татах
  UnitExportExcel = async (SearchOption, callback) => {
    const Token = localStorage.getItem("MnCardioToken");
    await Server({
      method: "POST",
      url: "/CVDReport/UnitExportExcel",
      ...
      .then((res) => {
        ...
        if (Data.Success === false) {
          ...
          saveAs(xlsxBlob, "NegdsenBurtgel.xlsx");
          callback && callback();
        }
      })
      .catch((err) => {
        ...
        >
          ...
          <Actions
            download={async (callback) => {
              ...
              await this.UnitExportExcel(
                SearchOption,
                () => callback && callback(),
              );
            }}
            refresh={async (callback) => {
              await this.GetData(this.SearchOption);
              callback && callback();
            }}
          />
        </div>
