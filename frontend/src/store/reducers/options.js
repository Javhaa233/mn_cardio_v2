...
export const validationRules = {
  invDate: [{ type: "required", message: i18n.t("Баримтын огноо оруулна уу") }],
  stockId: [{ type: "required", message: i18n.t("Агуулах оруулна уу") }],
  stockToId: [
    { type: "required", message: i18n.t("Хүлээн авах агуулах сонгоно уу") },
  ],
  branchId: [{ type: "required", message: i18n.t("Салбар сонгоно уу") }],
};

export const getLookup = (options) => async (dispatch) => {
  dispatch(setLoading(true));
  try {
    const responseData = await lookup({ url: "/base/Options/lookup", options });
    ...
    dispatch(
      setAlert({
        success: false,
        message: i18n.t("Мэдээ татахад алдаа гарлаа"),
      }),
    );
    return { data: [], totalCount: 0 };
  }
...
export const getOne = (id, select) => async (dispatch) => {
  dispatch(setLoading(true));
  try {
    const responseData = await one({ objectName: "Options", id, select });
    dispatch(setLoading(false));
    if (responseData.success) {
    ...
    console.log({ err });
    dispatch(setLoading(false));
    dispatch(setAlert({ success: false, message: i18n.t("Алдаа гарлаа") }));
    return false;
  }
};
...
export const getList = (options) => async (dispatch) => {
  dispatch(setLoading(true));
  try {
    const responseData = await list({ objectName: "Options", options });
...
export const update =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      if (responseData.success) {
        dispatch(
          setAlert({ success: true, message: i18n.t("Амжилттай хадгаллаа") }),
        );
        return true;
      } else {
        dispatch(handleResponseError(responseData));
...
export const destroy = (id) => async (dispatch) => {
  ...
  try {
    ...
    if (responseData.success) {
      dispatch(
        setAlert({ success: true, message: i18n.t("Амжилттай устгалаа") }),
      );
      return true;
    } else {
      dispatch(handleResponseError(responseData));
