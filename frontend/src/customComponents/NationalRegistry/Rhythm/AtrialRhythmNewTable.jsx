...
import AtrialRhythmNew from "customComponents/DetailViews/NationalRegistry/Rhythm/AtrialRhythmNew";

// helper
import Helper from "helper";

class AtrialRhythmNewTable extends BaseList {
  constructor(props) {
    super(props);
    const { t } = this.props;
    ...
    // refs
    this.AtrialRhythmNewRef = createRef();
  ...
  ShowData = (data) => {
    this.setState({
      DialogData: (
        ...
        >
          <AtrialRhythmNew
            ref={(ref) => (this.AtrialRhythmNewRef = ref)}
            DataId={data.Id}
          />
        </BaseDialog>
      ),
  ...
  ExportExcel = async () => {
    const ObjectName = "AtrialRhythmNew";
    if (!ObjectName) {
      const { t } = this.props;
    ...
    return (
      ...
      >
        ...
        >
          ...
          <UniCard title={t("Тосгуурын жирвэгнээ (Шинэ)")} padding={15}>
            <GridToolbar>
              <ToolbarField>
                <BaseLookUpGridLoad
                  disabled={false}
