...
Both repos have `node_modules` installed (~815 MB together), and Puppeteer's Chrome is downloaded.
...
- Port is `process.env.PORT || 5001` (`server.js:596`). Health check: `GET /health`.
...
- `Config-Template.env` was incomplete; `REPORT_DIR`, `XYP_KEY`, `XYP_TOKEN`, `REGNUM` and `PORT`
  ...
  `MAIL_SMTP_SECURE`. `SIGNATURE_HOST` looks required but is read by **no code at all** (digital
  signature is not implemented). `DbConnection.js` reads `SQL_SERVER`, `SQL_PORT` and `SQL_ENCRYPT`
  (none of them in the template) and does **not** use `SQL_HOST` — see `docs/handover/admin-guide.md` §3.
...
- The server **exits** if it cannot create `ALLFILE_DIR` or `REPORT_DIR`. Both are picked per platform;
  ...
  `/home/admin630/Desktop/outReports/` on Linux (`server.js:42-95`).

**Frontend** (from the frontend root):

```
npm install
npm run dev        # http://localhost:3000, proxies 53 prefixes to 127.0.0.1:5001
...
Routes are declared in two tables in `server.js`: `routeGroups.public` (5 prefixes) and
`routeGroups.protected` (49 prefixes, wrapped in `Auth.verifyToken`). Nearly everything is **POST**.
The full inventory is `backend/docs/API-WEB.md` — **397 endpoints** as of 2026-09-16 — regenerated by
`node scripts/generate_api_reference.js`. Take the number from the file, not from here. Express matches paths case-insensitively, so the
protected `/api/Report` prefix also swallows `/api/report/*` from `api/`: it is token-protected by
...
**`backend/config/Config.env` does not point at MnCardio_test.** `generate_form_views.js` and the
server both load it, and the generator runs `CREATE OR ALTER VIEW` on whatever it reaches. To work
against the test database, set `SQL_SERVER`/`SQL_DB`/`SQL_USER`/`SQL_PASSWORD`/`SQL_PORT`/
`SQL_ENCRYPT=true` from `test-environment.env` in the process first (dotenv never overrides a set
variable) and check `SELECT DB_NAME()` through `config/DB` before running anything.

Seeded so far — **eleven forms, 1,688 dictionary rows**: 1.1 (66), **1.2 (75)**, 1.3 (62),
1.5 (178), 1.6 (196), 1.7 (218), 1.8 (234), 1.9 (219), **2.1 (30)**, **2.2 (244)**, 3.1 (164 active —
`SAVE` and `COMPLETEANDSAVE` were the tender describing the two buttons, and are retired).
...
- **Types were inferred and some are wrong.** `scripts/fix_form_field_types_and_required.sql`
  ...
  `scripts/fix_form_conditional_links_and_sections.sql` (2026-09-11) fixes the second batch: dates
  on 2.2/3.1 seeded as radios over `{dd, mm}` or `{Тодорхойгүй, Үгүй}`, 2.2 blood pressure / heart
  rate / echo measurements seeded as the same radio, 2.2 medications with no "Тийм" option, parents
  pointing at the wrong question or the wrong code, and 3.1's sections. Both scripts are applied to
  MnCardio_test only; production needs them with the customer's approval.
- **Only 5 of the 1,687 rows carried `IsRequired`**, so there was almost nothing for validation to
  ...
  is a clinical decision (tracker row 23), not ours. Note 3.1's only required field,
  `DateOfAdmissionTo`, sits in the ST+ register, so a non-ST+ record has no required field at all —
  raised in `ZSUT-followup-2026-09-11.md`.
...
**Conditional reveal is data.** A row's `ParentField` + `ParentValue`, where `ParentValue` is `o2`
(equals), `a|b` (any of) or `!o1` (answered, and not o1). Visibility is **transitive**: a field whose
parent is hidden is hidden, so a whole block hides by giving only its top rows a parent. One rule,
two copies that must stay identical — `backend/helper/TenderFormVisibility.js` (print, Confirm) and
`IsVisible` in `TenderForm.jsx`. Parent values are OPTION CODES, never labels: check the parent's
option set (`yorn` is `y`/`n`, the generated `f31_*` sets code Yes as `o1`, `f22_14` as `o2`). The
seed wrote `'y'` against `o1`-coded lists and those children could never appear. 3.1 uses this to
skip the angioplasty sections (`DecisionForCAG` = `!o1`, tender L8330) and to reveal the ST+
register (`AnginaStatus` = `o4`, L8492). Answers behind a hidden question are cleared at save.

**A confirmed record is locked for everyone, permanently** — tender 3.1 L8599–8605, and the
customer's decision on 2026-09-11 that there is no unlock. `CustomSave`, `Confirm` and `Delete` all
go through `LoadWritable`: a confirmed record is refused, and another organization's record is
refused unless the caller is role 1. The form's own bar has "Дуусгаж хадгалах" (save, check,
confirm). A repeat procedure is a NEW record: `AllowDuplicate` on the `TenderForm` registry (3.1
only) offers to copy the latest one, minus its dates, times and required identity fields. Confirm
checks required fields on the server; save checks `Number`/`Date` values.

**Organization is on the doctor profile.** `Users` has no `OrganizationId` column; it is
`DoctorsProfile.OrganizationId`, reached as `req.LogedUser.Doctor.OrganizationId`. Anything reading
`LogedUser.OrganizationId` gets undefined — TenderFormData was saved with NULL for that reason until
2026-09-11 (`scripts/backfill_tenderformdata_organization.sql`).

**Scores and intervals the tender marks as calculated** live in `DERIVED` in `TenderForm.jsx` —
CHADS2-VASc on 2.2, and the Pain→ECG, ECG→wire, ECG→TIV minutes on 3.1 — beside `AUTOCALC`
(BMI, BSA). They are read-only and saved with the record. `BaseSimpleDate` is not used for
dictionary dates: it has no label and shows today's date in an empty field.
...
**Rewritten 2026-09-16 after a whole-system audit.** Most of what this section used to list has
been fixed; leaving it as-was sent people at solved problems and made the parts that are still
true harder to believe. What was closed, and by what, is at the bottom.
...
- **`ssh.env` and `test-environment.env` in the project root hold live SQL and SSH credentials
  ...
  on this disk.
- **Credentials that were committed are compromised and not yet rotated.** The TLS private key
  for `*.telemedicine.mn`, the ХУР signing key (`.key`, `.pem`, `.ovpn`) and three populated
  `.env` files were tracked and pushed to GitLab. They have been untracked and `.gitignore`
  widened, **but removing a file does not remove it from git history** — every one of
  `SQL_PASSWORD`, `JWT_PASS`, `MAIL_USER_PASS`, `EMD_PASSWORD` and `XYP_PASSWORD` needs
  rotating by ЗСҮТ. `ZSUT-credential-rotation-2026-09-16.md` is the request, with the order to
  do them in. `helper/MediaTicket.js:38` already says `JWT_PASS` is in history and unrotated.
- **`express.json` runs at a 100 MB limit** (`JSON_BODY_LIMIT`, `helper/FeatureFlags.js`), for
  both JSON and urlencoded, with `parameterLimit: 100000`. Deliberate — the registry forms post
...
- **Passwords in plaintext.** `controllers/patient-care/OutPatientInfoController.js:218` logs a
  ...
  resets a patient's password and returns the cleartext — by design, audited, but reachable by
  any staff token that passes its in-handler role check. `model/BaseModel/PatientUsers.js`
  declares a `PlainPassword` column; nothing writes it, but `helper/Auth.js` explicitly deletes
  it from the session, so the table is expected to contain them.
- **The dev auth bypasses still exist**, now double-gated. `helper/Auth.js:250` and
  `controllers/auth/UserController.js:294` require `NODE_ENV` *and* `Flags.AllowInsecureDevAuth`,
  which defaults false and prints a red banner at boot when true. One environment variable
  re-opens both.
- **`index.html` loads two third-party assets at runtime** — FontAwesome CSS and one Google
  Fonts request — both render-blocking on the login path. FontAwesome backs six icons that all
  have MUI equivalents already imported elsewhere.
- **No observability.** No logger of any kind (810 `console.*` calls, 468 in controllers), and
  no client-side error reporting. `server.js` monkey-patches `console.log` in production to drop
  anything whose first argument is not an `Error` and does not contain "error"/"exception"/"fail",
  so classification is by substring and most controller diagnostics vanish. On any host that is
  not labelled `production`, the opposite: 42 calls `JSON.stringify` whole response payloads —
  patient records included — into the log.

#### Closed since this section was written — do not re-fix these

| Was | Now |
|---|---|
| `helper/Auth.js` skips `jwt.verify()` in development | Double-gated behind `ALLOW_INSECURE_DEV_AUTH`, default false |
| `UserController.js` skips password validation when `NODE_ENV !== 'production'` | Same gate; password checking is the default branch |
| `/api/base/*` and `/api/report/*` mounted outside `Auth.verifyToken` | `api/index.js` gates both with `VerifyTokenJson` + `DenyPatient` |
| `TestController` public, incl. a 1 GB unauthenticated `PUT /uploadFile` | Four routes deleted 2026-09-14; the two left are pure string→boolean regex checks |
| No rate limiting anywhere | `helper/RateLimit.js` built (count-only by default, see above) |
| No CORS / helmet | `cors(corsOptions)` + `helmet()` in `server.js` |
| `.gitignore` is three lines | Now covers `.env*`, `*.key`, `*.pem`, `*.pfx`, `*.ovpn`, `id_rsa*` anywhere |
| `index.html` loads five CDN assets | Down to two; html2canvas, jvectormap, chartist and dataTables removed |
| SQL injection was not recorded here at all | ~28 request-controlled sites parameterised 2026-09-16 |
| `POST /api/XypService/testCall` was public | Protected + `RoleId 1`; it signs ХУР requests with the hospital's own key |
| `NODE_TLS_REJECT_UNAUTHORIZED=0` shipped in `Config-Template.env` | Removed. It disabled certificate verification for **every** outbound HTTPS call — ХУР, ЭМД, SMTP — and was live via `config/DbConnection.js`'s bare `dotenv.config()` |
| CORS allowed any origin whenever `NODE_ENV !== 'production'`, with `credentials: true` | Development now means the allowlist plus any loopback port; `CORS_ALLOW_ANY_ORIGIN` is the opt-in escape hatch |
...
- `build/` exists on disk but is **not** tracked — `/build` is in `frontend/.gitignore`. It is not source.

### Typos baked into the code — match them exactly or nothing resolves

| Wrong-looking | Where | Note |
|---|---|---|
| `SingelSelect` | ModelConfigs | 57 uses alongside 156 correct `SingleSelect` |
...
- **Abandoned migration.** 33 files are stubs whose bodies are commented out with
...
- **172 class components** (`extends Component` / `extends BaseCustomForm`) coexist with hooks-based
