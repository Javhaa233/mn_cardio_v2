...
const CardiacRhythm = require('../../reports/CardiacRhythm');
  ...
  try {
    ...
    if (LogedUser && PatientId && PatientRegNo) {
      const [LastData, data] = await sequelize.query(
        "SELECT TOP 1 Id FROM CardiacRhythm WHERE PatRegNo=N'" +
          PatientRegNo +
...
async function CustomSave(req, res) {
  try {
    ...
    const LogedUser = req.LogedUser;
    const { Id, PatientId, PatientRegNo } = req.body;
    const Data = JSON.parse(req.body.Data);

    if (PatientId && PatientRegNo && LogedUser && Data) {
      //create vascular disease
      let CardiacRhythmId = null;
      if (Id) {
        CardiacRhythmId = await BaseControllerHelper.BaseUpdate({
          ObjectName: 'CardiacRhythm',
          Data: { ...Data, Id },
          LogedUser,
          SaveLog: true,
        ...
        // create CardiacRhythm
        CardiacRhythmId = await BaseControllerHelper.BaseCreate({
          ObjectName: 'CardiacRhythm',
          Data: {
            ...Data,
            is_confirm: 'no',
      ...
      result.Data = { DataId: CardiacRhythmId };
      return res.send(JSON.stringify(result));
    } else {
      return res.send(
        JSON.stringify(BaseControllerHelper.GetDefaultErrorResult('Information is missing'))
...
async function Confirm(req, res) {
  try {
    ...
    if (LogedUser && Id) {
      const Disease = await Models.CardiacRhythm.findOne({
      ...
      await Models.CardiacRhythm.update({ is_confirm: 'yes' }, { where: { Id } });
