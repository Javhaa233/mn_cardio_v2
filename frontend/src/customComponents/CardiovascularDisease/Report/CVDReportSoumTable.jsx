...
import {
  HeaderArray,
  RowColumns,
} from "assets/store/reportSoumTableColumns.jsx";
...
class CVDReportSoumTable extends Component {
  constructor(props) {
    super(props);
  ...
  // Сумын мэдээлэл харах
  SoumReportExportExcel = async (SearchOption, callback) => {
    const Token = localStorage.getItem("MnCardioToken");
    await Server({
      method: "POST",
      url: "/CVDReport/SoumReportExportExcel",
      ...
      .then((res) => {
        ...
        if (Data.Success === false) {
          ...
          saveAs(xlsxBlob, "SoumReport.xlsx");
          callback && callback();
        }
      })
      .catch((err) => {
        ...
        >
          ...
          <Actions
            download={async (callback) => {
              ...
              await this.SoumReportExportExcel(
                SearchOption,
                () => callback && callback(),
              );
            }}
