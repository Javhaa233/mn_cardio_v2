...
And three folders you can run rather than read:

| Folder | What it gives you |
|---|---|
| [`app/`](app/) | **The Flutter app itself** — patient and doctor modules, both login sections. |
...
| [`client/`](client/) | `smoke.js` — ten calls that prove your setup, with no dependencies. `dart/` — the `dio` client, token storage and typed wrappers. |

**The app now exists: [`app/`](app/).** One Flutter app with two login sections,
as tender §33 requires: the **patient module** (§2.1–2.7) and the **doctor
module** (§1.1–1.5), plus login, chat and reminders. Which shell opens is decided
by the `RoleId` the server returns, not by the login-screen choice.

Start at [app/README.md](app/README.md) for requirement-by-requirement coverage
and the known gaps, and [app/SETUP.md](app/SETUP.md) to get it running —
`android/` and `ios/` are generated by `flutter create` on first setup, not
committed.

> **`client/dart/` and `app/lib/core/network/` are two different things.**
> `client/dart/` is the reference client that came with the handover notes — read
> it to check a call shape. The app does **not** import it: `app/lib/core/` has its
> own `dio` client with the same envelope rules plus the interceptors, secure token
> storage and Mongolian error mapping the app needs. If the two ever disagree about
> an endpoint, the backend source wins and both should be corrected.
