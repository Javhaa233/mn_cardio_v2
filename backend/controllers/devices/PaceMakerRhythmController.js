...
async function Confirm(req, res) {
  try {
    ...
    if (LogedUser && Id) {
      const PM = await Models.PaceMakerRhythm.findOne({
        where: { Id },
        raw: true,
      });
      if (!PM) {
      ...
      const CreateUserId = PM.CreateUserId;
      ...
      await Models.PaceMakerRhythm.update({ is_confirm: 'yes' }, { where: { Id } });
  ...
  try {
    ...
    if (LogedUser && PatientRegNo) {
      const [LastData, data] = await sequelize.query(
        "SELECT TOP 1 Id FROM PaceMakerRhythm WHERE PatRegNo=:PatientRegNo AND (is_confirm='no' OR is_confirm IS NULL) ORDER BY Id DESC",
...
async function CustomSave(req, res) {
  try {
    ...
    if (PatientRegNo && LogedUser && Data) {
      //create hf ambulance
      let PaceMakerRhythmId = null;
      if (Id) {
        PaceMakerRhythmId = await BaseControllerHelper.BaseUpdate({
          ObjectName: 'PaceMakerRhythm',
          Data: { ...Data, Id },
          LogedUser,
          SaveLog: true,
        ...
        // create PaceMakerRhythm
        PaceMakerRhythmId = await BaseControllerHelper.BaseCreate({
          ObjectName: 'PaceMakerRhythm',
          Data: {
            ...Data,
            is_confirm: 'no',
      ...
      result.Data = { DataId: PaceMakerRhythmId };
      return res.send(JSON.stringify(result));
    } else {
      return res.send(
        JSON.stringify(BaseControllerHelper.GetDefaultErrorResult('Information is missing'))
